...
function AuthComponent() {
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    try {
      const userCred = isRegister ? await createUser(email, password) : await loginUser(email, password);
      alert(userCred);
      navigate('/editor');
    } catch (error) {
      alert(error);
  ...
  return (
    <div className="h-screen flex">
      <div className="w-full relative flex">
        ...
        <form className="absolute mezzo flex flex-col" onSubmit={handleSubmit}>
          ...
          <button className={`border-2 ${isRegister ? "bg-indigo-300 text-indigo-900 hover:bg-purple-300 hover:text-purple-900" : "hover:bg-indigo-300 hover:text-indigo-900 bg-purple-300 text-purple-900"}  rounded transition duration-500 ease-in-out font-bold mt-2 p-2 uppercase`}
            type="submit">{isRegister ? "Signup" : "Login"}</button>
        </form>
